...
页面通过同源代理调用图片接口，避免浏览器直连三方服务商时遇到 CORS 限制。页面选择服务商类型和创作参数，`Base URL`、`Model` 由后端按服务商分组配置，`API Key` 暂时从页面输入并随请求传给后端代理。
...
GEMINI_IMAGE_BASE_URL="https://vip.undyingapi.com" \
...
- `GPT Image`: 使用后端 `OPENAI_IMAGE_BASE_URL` / `OPENAI_IMAGE_MODEL` 配置，API Key 从页面输入。
- `Gemini / Nano Banana`: 使用后端 `GEMINI_IMAGE_BASE_URL` / `GEMINI_IMAGE_MODEL` 配置，API Key 从页面输入。
...
| `OPENAI_IMAGE_API_KEY` | 可选备用密钥；页面输入的 API Key 会优先使用 |
| `OPENAI_IMAGE_MODEL` | OpenAI-compatible 图片模型，默认 `gpt-image-2` |
| `GEMINI_IMAGE_BASE_URL` | Nano Banana 服务地址，默认 `https://vip.undyingapi.com` |
| `GEMINI_IMAGE_API_KEY` | 可选备用密钥；页面输入的 API Key 会优先使用 |
...
docker run -d \
  ...
  -p 8787:8787 \
  -v "$(pwd)/logs:/app/logs" \
  -e GEMINI_IMAGE_BASE_URL="https://vip.undyingapi.com" \
  -e GEMINI_IMAGE_MODEL="gemini-2.5-flash-image-preview" \
